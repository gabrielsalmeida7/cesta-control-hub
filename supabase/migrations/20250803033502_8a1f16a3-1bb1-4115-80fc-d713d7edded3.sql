-- Passo 1: Corrigir as Funções RLS para suportar usuários bypass
...
BEGIN
  ...
  -- Admin bypass user: d1e6f7a2-b3c4-5d6e-7f8a-9b0c1d2e3f4g
  IF user_id = 'd1e6f7a2-b3c4-5d6e-7f8a-9b0c1d2e3f4g'::uuid THEN
    RETURN 'admin'::user_role;
  END IF;
  
  -- Institution bypass user: a1b2c3d4-e5f6-7890-abcd-ef1234567890
...
AS $$
  SELECT user_id IN (
    'd1e6f7a2-b3c4-5d6e-7f8a-9b0c1d2e3f4g'::uuid,  -- admin bypass
    'a1b2c3d4-e5f6-7890-abcd-ef1234567890'::uuid    -- institution bypass
  );
$$;

-- Comentar as funções para documentação
COMMENT ON FUNCTION public.get_user_role(uuid) IS 'Retorna o role do usuário com fallbacks para usuários bypass';
COMMENT ON FUNCTION public.get_user_institution(uuid) IS 'Retorna o institution_id do usuário com fallbacks para usuários bypass';
COMMENT ON FUNCTION public.is_bypass_user(uuid) IS 'Verifica se o usuário é um usuário bypass para testes';