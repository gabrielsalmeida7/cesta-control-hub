...
import { sharePdfFromUri } from '@/utils/shareFile';
...
export const useConsentManagement = () => {
  ...
  const generateTerm = useCallback(async (
  ...
  ): Promise<{ pdfUri: string; termId: string } | null> => {
    ...
    try {
      ...
      const pdfUri = await generateConsentTermPDF(termData);
   ...
   */
  const downloadTerm = useCallback(async (pdfUri: string, familyName: string, termId: string) => {
    try {
      await sharePdfFromUri(
        pdfUri,
        `Termo_Consentimento_${familyName.replace(/\s+/g, '_')}_${termId}.pdf`
      );
    } catch (shareError) {
      console.warn('Falha ao compartilhar termo:', shareError);
      toast({
        title: "Termo gerado",
        description: "O PDF foi criado, mas o compartilhamento falhou. Tente novamente.",
      });
    }
  }, [toast]);
