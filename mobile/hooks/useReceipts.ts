...
export const useGenerateMovementReceipt = () => {
  ...
  return useMutation({
    mutationFn: async (movementId: string) => {
      ...
      // Gerar PDF
      const pdfUri = await generateStockMovementReceipt(
        movement,
    ...
    onSuccess: async (data) => {
      ...
      try {
        await sharePdfFromUri(data.pdfUri, 'recibo-movimentacao.pdf');
      } catch (shareError) {
        console.warn('Falha ao compartilhar recibo:', shareError);
        toast({
          title: "Recibo gerado",
...
export const useGenerateDeliveryReceipt = () => {
  ...
  return useMutation({
    mutationFn: async (deliveryId: string) => {
      ...
      // Gerar PDF
      const pdfUri = await generateDeliveryReceipt(
        delivery,
        items,
    ...
    onSuccess: async (data) => {
      ...
      try {
        await sharePdfFromUri(data.pdfUri, 'recibo-entrega.pdf');
      } catch (shareError) {
        console.warn('Falha ao compartilhar recibo:', shareError);
        toast({
          title: "Recibo gerado",
